...
from kavyamusic import app
from kavyamusic.core.call import ankit
from kavyamusic.misc import db
from kavyamusic.utils.database import get_assistant, get_authuser_names, get_cmode
from kavyamusic.utils.decorators import ActualAdminCB, AdminActual, language
from kavyamusic.utils.formatters import alpha_to_int, get_readable_time
...
async def restartbot(client, message: Message, _):
    ...
    try:
        db[message.chat.id] = []
        await ankit.stop_stream_force(message.chat.id)
    except:
        pass
    ...
    if chat_id:
        ...
        try:
            db[chat_id] = []
            await ankit.stop_stream_force(chat_id)
        except:
            pass
             ...
             [
                 [
                      InlineKeyboardButton(
                         "• 𝖧ᴀᴄᴋᴇᴅ вʏ  •", url=f"https://t.me/ll_ankit_BABY_lll")
                 ]
            ]
         ),
     )
